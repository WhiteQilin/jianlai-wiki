...
- `images/textures/` - Subtle paper, ink, or noise overlays (if image-based textures are needed).
...
3. **Usage**: Reference these files in markdown or components starting with a forward slash: `/images/characters/chen-pingan.jpg` (VitePress resolves `/` to the `public/` directory).
